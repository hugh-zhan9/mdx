...
- Workspace stale-WebView recovery may auto-reload only when the cached workspace snapshot has a root path and no dirty tabs; if any cached tab is dirty, native recovery must skip reload to avoid losing edits that may not have reached delayed draft autosave.
