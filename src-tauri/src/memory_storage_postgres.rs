    ...
    fn insert_memory(&mut self, memory: &StoredMemoryWrite) -> Result<bool, WorkspaceError> {
        ...
        self.client
            .execute(
                ...
                VALUES ($1, $2, $3, $4, $5, 'active', $6::jsonb, $7, $8, $9, $9, NULL)
    ...
    fn insert_inbox_candidate(&mut self, inbox: &StoredInboxWrite) -> Result<bool, WorkspaceError> {
        ...
        self.client
            .execute(
                ...
                VALUES ($1, $2, $3, $4, $5, 'pending', $6::jsonb, $7, $8, NULL, $9, NULL)
            ...
            .map_err(|error| {
                postgres_error(
                    "memory_postgres_inbox_insert_failed",
                    "failed to insert postgres inbox candidate",
                    error,
                )
            })
    }
